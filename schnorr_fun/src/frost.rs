...
impl FrostKey {
    ...
    pub fn tweak(&mut self, tweak: Scalar<impl Secrecy, impl ZeroChoice>) -> Option<Self> {
        let joint_public_key = g!(self.joint_public_key + self.tweak * G)
            .normalize()
            .mark::<NonZero>()?;
    ...
    pub fn into_bip340_key(self) -> Bip340FrostKey {
...
/// A [`FrostKey`] that has been converted into a [`Bip340FrostKey`] key.
///
...
/// Tweaks applied to a `Bip340FrostKey` are XOnly tweaks to the joint public key
...
mod test {
    ...
    proptest! {
        ...
        fn frost_prop_test((n_parties, threshold) in (3u32..8).prop_flat_map(|n| (Just(n), 3u32..=n)), tweak1 in option::of(any::<Scalar<Public, Zero>>()), tweak2 in option::of(any::<Scalar<Public, Zero>>())) {
            ...
            let (secret_shares, frost_keys): (Vec<Scalar>, Vec<Bip340FrostKey>) = (0..n_parties).map(|i| {
                ...
                .unwrap();
                let mut bip_340_frost_key = frost_key.into_bip340_key();

                // apply some xonly tweaks
                for tweak in [tweak1, tweak2] {
                    if let Some(tweak) = tweak {
                        bip_340_frost_key = bip_340_frost_key.tweak(tweak).unwrap();
                    }
                }
                (secret_share, bip_340_frost_key)
            }).unzip();
    ...
    fn frost_test_end_to_end() {
        ...
        let mut bip340_frost_key = frost_key.into_bip340_key();
        let mut bip340_frost_key2 = frost_key2.into_bip340_key();
        let mut bip340_frost_key3 = frost_key3.into_bip340_key();

        assert_eq!(bip340_frost_key, bip340_frost_key2);
        assert_eq!(bip340_frost_key, bip340_frost_key3);
        ...
        bip340_frost_key = bip340_frost_key.tweak(tweak.clone()).expect("tweak worked");
        bip340_frost_key2 = bip340_frost_key2
            .tweak(tweak.clone())
            .expect("tweak worked");
        bip340_frost_key3 = bip340_frost_key3.tweak(tweak).expect("tweak worked");
        ...
        let verification_shares_bytes: Vec<_> = bip340_frost_key
            .verification_shares
            .iter()
            .map(|share| share.to_bytes())
            .collect();
        ...
        let sid1 = [
            bip340_frost_key.joint_public_key.to_bytes().as_slice(),
            verification_shares_bytes.concat().as_slice(),
        ...
        let session =
            frost.start_sign_session(&bip340_frost_key, nonces, Message::plain("test", b"test"));
        ...
        let sig1 = frost.sign(&bip340_frost_key, &session, 0, &secret_share1, nonce1);
        let sig3 = frost.sign(&bip340_frost_key3, &session, 2, &secret_share3, nonce3);

        dbg!(sig1, sig3);

        assert!(frost.verify_signature_share(&bip340_frost_key, &session, 0, sig1));
        assert!(frost.verify_signature_share(&bip340_frost_key, &session, 2, sig3));
        let combined_sig =
            frost.combine_signature_shares(&bip340_frost_key, &session, vec![sig1, sig3]);
