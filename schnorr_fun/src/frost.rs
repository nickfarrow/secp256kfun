...
mod test {
    ...
    proptest! {
        #[test]
        fn frost_prop_test((n_parties, threshold) in (3u32..8).prop_flat_map(|n| (Just(n), 3u32..=n)), tweak1 in option::of(any::<Scalar<Public, Zero>>()), tweak2 in option::of(any::<Scalar<Public, Zero>>()), signer_mask_seed in array::uniform32(1u8..)) {
            let frost = Frost::new(Schnorr::<Sha256, Deterministic<Sha256>>::new(
                Deterministic::<Sha256>::default(),
            ));
            ...
            signer_mask.shuffle(&mut TestRng::from_seed(RngAlgorithm::ChaCha, &signer_mask_seed));
